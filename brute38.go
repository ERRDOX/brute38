...
Specifying a key and a set of passwords to try:
  ...
  <pwlen_or_pat> The length of the range of passwords to try.  This is the
                 number of characters in the password that will be tried.

                 Length, in characters, of the original passphrase. Cracking
...
func main() {
	...
	for currentLength := 1; currentLength <= pwlen; currentLength++ {
		startTime := time.Now()
		fmt.Printf("\nCurrent Password length: %d.\n", currentLength)
		result := bip38.BruteChunk(ncpu, priv, charset, currentLength, pat, lines, chunk, chunks, resume, [2]byte{coin.networkVersion, coin.privateKeyPrefix}, coin.name)
		if result == "" {
			fmt.Printf("\nNot found in len %d.\n", currentLength)
